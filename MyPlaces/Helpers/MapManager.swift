...
class MapManager {
        ...
        case .denied:
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.showAlert(title: "Your Location in not available",
                               message: "Please go to: Setting -> MyPlaces -> Location")
            }
            break
        case .notDetermined:
    ...
    //Строим маршрут от местоположения пользователя до места
    func getDirections(for mapView: MKMapView, previousLocation: (CLLocation) -> ()) {
        ...
        directions.calculate { (response, error) in
            ...
            guard let response = response else {
                self.showAlert(title: "Error", message: "Direction is now available")
                return
            }
    ...
    //Сброс всех ранее построенных маршрутов перед постронением нового
